...
const E2E_LLM_MODEL = 'HuggingFaceTB/SmolLM2-360M-Instruct';
...
test('chat answers the "What can you do?" prompt', async ({ page }) => {
  ...
  if (await downloadDialog.waitFor({ state: 'visible', timeout: 10_000 }).then(() => true).catch(() => false)) {
    await downloadDialog.getByRole('button', { name: 'Download' }).click();
  }

  await expect(page.getByTestId('chat-message-content-user')).toContainText('What can you do?');
  ...
  await expect
    .poll(async () => (await assistantResponse.textContent())?.trim() ?? '', {
      timeout: 10 * 60 * 1000,
      message: 'Wait for the local AI model to produce a non-empty chat response',
    })
    .not.toEqual('');
});
