...
export function buildLeaderboard(game: Game): { rank: number; player: Player; total: number; delta: number }[] {
...
export type ToolArgs = {
  ...
  navigate: { view: 'scoring' | 'leaderboard' | 'chart' | 'table' | 'chat' };
};
  ...
  switch (name) {
    case 'add_round': {
      ...
      for (const player of game.players) {
        const match = Object.entries(scores).find(([k]) => {
          const p = resolvePlayer(k, game.players);
    ...
    case 'update_round': {
      ...
      for (const [name, pts] of Object.entries(scores)) {
        const p = resolvePlayer(name, game.players);
    ...
    case 'get_leaderboard': {
      ...
      return { success: true, message: 'Leaderboard retrieved.', data: lb };
    }
